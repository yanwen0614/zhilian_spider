...
def statis(job):
    ...
    i = 0
    ttpp = 0
    outlierfile = open(r'./_findoutlier/outlier', 'w', encoding='utf-8')
        ...
        with open(txtfile_name.format(city=city, job=job), mode='r', encoding='utf-8') as f:
            ...
            while 1:
                ...
                if line["公司"] in companys:
                    ...
                    if companys[line["公司"]].get_size_tag == 0:
                        companys[line["公司"]].size = companys[line["公司"]].getsize(line["url"])
                else:
                    companys[line["公司"]] = company(line["公司"], location, city, need_people,line["url"])
                tag = [0, 0]
...
if __name__ == '__main__':
    job = '房地产'
    print(job)
    companys, ttpp = main(job)
    companyfile = open(r'./_findoutlier/company', 'w+', encoding='utf-8')
    for item in list(sorted(companys.items(), key = lambda item: -item[1].need_people))[:1000]:
        item[1].write(companyfile)

    companyspick = open(u'.\_result\{job}\{job}-companys.pickle'.format(job=job), 'wb')
    pickle.dump(companys, companyspick)
    print(ttpp)
    print(getneedpeople(1000, companys))
